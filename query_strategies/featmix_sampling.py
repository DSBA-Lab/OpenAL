...
class AlphaMixSampling(Strategy):
   ...
   def query(self, model, n_subset: int = None) -> np.ndarray:
      ...
      out = self.model.forward_head(var_emb)
      ...
      if candidate.sum() > 0:
         ...
         c_alpha = F.normalize(F.adaptive_avg_pool2d(org_ulb_embedding,1)[candidate].view(candidate.sum(), -1), p=2, dim=1).detach()
   ...
   def find_candidate_set(self, lb_embedding, ulb_embedding, pred_1, ulb_probs, alpha_cap, Y, grads):
      ...
      for i in range(n_label):
         ...
         alpha = self.calculate_optimum_alpha(alpha_cap, embed_i, ulb_embed.view(ulb_embed.size(0),-1), grads.view(ulb_embed.size(0),-1))

         embedding_mix = (1 - alpha) * ulb_embed.view(ulb_embed.size(0),-1) + alpha * embed_i
         out = self.model.forward_head(embedding_mix.view(ulb_embed.size(0),ulb_embed.size(1),ulb_embed.size(2),ulb_embed.size(3)))
         out = out.detach().cpu()
         alpha = alpha.cpu()
